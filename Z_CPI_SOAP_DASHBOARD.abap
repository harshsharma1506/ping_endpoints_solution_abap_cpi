...
CLASS lcl_test_dashboard DEFINITION FOR TESTING
  DURATION SHORT
  RISK LEVEL HARMLESS.

  PRIVATE SECTION.
    DATA: mo_cut TYPE REF TO lcl_dashboard.
    METHODS setup.
    METHODS test_map_status_ok FOR TESTING.
    METHODS test_map_trend_improving FOR TESTING.
    METHODS test_map_trend_worsening FOR TESTING.
    METHODS test_map_trend_steady FOR TESTING.
ENDCLASS.

CLASS lcl_test_dashboard IMPLEMENTATION.
  METHOD setup.
    mo_cut = NEW lcl_dashboard( ).
  ENDMETHOD.

  METHOD test_map_status_ok.
    DATA: lv_status TYPE string,
          lv_light  TYPE c.
    mo_cut->map_status( EXPORTING iv_msgty = 'S' IMPORTING ev_status = lv_status ev_light = lv_light ).
    cl_abap_unit_assert=>assert_equals( act = lv_status exp = 'OK' ).
    cl_abap_unit_assert=>assert_equals( act = lv_light  exp = '3' ).
  ENDMETHOD.

  METHOD test_map_trend_improving.
    cl_abap_unit_assert=>assert_equals(
      act = mo_cut->map_trend( iv_latest = 'S' iv_prev = 'E' )
      exp = '↑' ).
  ENDMETHOD.

  METHOD test_map_trend_worsening.
    cl_abap_unit_assert=>assert_equals(
      act = mo_cut->map_trend( iv_latest = 'A' iv_prev = 'S' )
      exp = '↓' ).
  ENDMETHOD.

  METHOD test_map_trend_steady.
    cl_abap_unit_assert=>assert_equals(
      act = mo_cut->map_trend( iv_latest = 'W' iv_prev = 'W' )
      exp = '→' ).
  ENDMETHOD.
ENDCLASS.
