...
CLASS lcl_test_monitor DEFINITION FOR TESTING
  DURATION SHORT
  RISK LEVEL HARMLESS.

  PRIVATE SECTION.
    DATA: mo_cut TYPE REF TO lcl_monitor.
    METHODS setup.
    METHODS test_classify_ok FOR TESTING.
    METHODS test_classify_auth FOR TESTING.
    METHODS test_classify_error FOR TESTING.
    METHODS test_classify_down FOR TESTING.
ENDCLASS.

CLASS lcl_test_monitor IMPLEMENTATION.
  METHOD setup.
    mo_cut = NEW lcl_monitor( ).
  ENDMETHOD.

  METHOD test_classify_ok.
    cl_abap_unit_assert=>assert_equals(
      act = mo_cut->classify_result( iv_status = 200 iv_error = '' )
      exp = 'OK' ).
  ENDMETHOD.

  METHOD test_classify_auth.
    cl_abap_unit_assert=>assert_equals(
      act = mo_cut->classify_result( iv_status = 401 iv_error = '' )
      exp = 'AUTH' ).
  ENDMETHOD.

  METHOD test_classify_error.
    cl_abap_unit_assert=>assert_equals(
      act = mo_cut->classify_result( iv_status = 500 iv_error = '' )
      exp = 'CPI_ERROR' ).
  ENDMETHOD.

  METHOD test_classify_down.
    cl_abap_unit_assert=>assert_equals(
      act = mo_cut->classify_result( iv_status = 0 iv_error = 'Error' )
      exp = 'DOWN' ).
  ENDMETHOD.
ENDCLASS.
